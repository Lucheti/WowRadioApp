...
class StationsViewController: UIViewController {
    ...
    func playLaunchGif() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: nil, queue: nil) { notification in
            self.seagueToNowPlaying()
        }

        player.play()
    }
    
    func seagueToNowPlaying() {
        let nowPlayingVC : NowPlayingViewController = (self.storyboard?.instantiateViewController(withIdentifier: "NowPlaying")) as! NowPlayingViewController;
        ...
        nowPlayingVC.load(station: radioPlayer.station, track: radioPlayer.track, isNewStation: true)
        nowPlayingVC.delegate = self
        self.present(nowPlayingVC, animated: true, completion: nil)
    }
    
    @IBSegueAction func goToNowPlaying(_ coder: NSCoder) -> NowPlayingViewController? {
        return NowPlayingViewController(coder: coder)
    }
...
//            // User clicked on row, load/reset station
//            radioPlayer.station = searchController.isActive ? searchedStations[indexPath] : stations[indexPath]
